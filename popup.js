...
const aiParseBtn = document.getElementById('ai-parse'); // may be absent if you didn't add AI Parse
...
startBtn.onclick = async () => {
  ...
  chrome.tabs.sendMessage(id, { type: 'SR_START' }, (resp) => {
...
stopBtn.onclick = async () => {
  const id = await activeTabId();
  if (!id) return;
  chrome.tabs.sendMessage(id, { type: 'SR_STOP' }, () => {
    setStatus('Stopped.');
...
fillBtn.onclick = async () => {
  ...
  const id = await activeTabId();
  if (!id) return;
  await chrome.tabs.sendMessage(id, { type: 'FILL_FIELDS', payload: obj });
  setStatus('Attempted to fill fields on page.');
};

// ---- Listen for transcript updates from content.js ----
chrome.runtime.onMessage.addListener((msg) => {
  if (msg?.type === 'SR_UPDATE') {
    const { interim, final } = msg.payload || {};
    const combined = [final, interim].filter(Boolean).join(' ');
    transcriptEl.value = combined.trim();
  }
  if (msg?.type === 'SR_ERROR') {
    setStatus('Mic error: ' + (msg.payload?.message || 'unknown'));
    startBtn.disabled = false;
    stopBtn.disabled = true;
  }
  if (msg?.type === 'SR_STATE') {
    const { listening } = msg.payload || {};
    startBtn.disabled = !!listening;
    stopBtn.disabled = !listening;
    if (!listening) setStatus('Stopped.');
  }
});
...
- Normalize numbers (e.g., '$5,000' for five thousand).
...
Return ONLY JSON.`;
  ...
  try { return JSON.parse(out); } catch { return transcriptToMap(text); }
}
